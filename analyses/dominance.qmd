...
Le tableau peut être affiché en français avec l'option `lang = "fr"`.

```{r}
ml |> guideR::tbl_contributions(decimals = 2, lang = "fr")
```
...
```{r}
mod |> 
  guideR::tbl_contributions(type = "II", lang = "fr") |> 
...
```{r}
mod |> 
  guideR::tbl_contributions(type = "I", lang = "fr") |> 
  gt::grand_summary_rows(
    columns = gt::any_of(c("Total", "Relative")),
    fns = Total ~ sum(.),
    fmt = ~ gt::fmt_percent(., decimals = 1)
...
modi |> 
  guideR::tbl_contributions(type = "III", lang = "fr")
```
...
```{r}
modi |> 
  guideR::tbl_contributions(type = "drop1", lang = "fr")
```
...
#| warning: false
mod |> 
  guideR::tbl_contributions(type = "add1", show = "Total", lang = "fr") |> 
  gt::grand_summary_rows(
