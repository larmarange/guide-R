...
::: callout-warning
Ce chapitre nécessite une version en cours de développement de `{broom.helpers}` installable avec la commande `remotes::install_github("larmarange/broom.helpers#202")`, ainsi qu'une version récente de `{gtsummary}` (version ≥ 1.6.3) et de `{ggstats}` (version ≥ 0.2.0).
:::
