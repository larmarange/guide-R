...
::: callout-warning
Ce chapitre nécessite une version en cours de développement de `{broom.helpers}` installable avec la commande `remotes::install_github("larmarange/broom.helpers")`, ainsi qu'une version récente de `{gtsummary}` (version ≥ 1.6.3) et de `{ggstats}` (version ≥ 0.2.1).
:::
...
Le tableau `pred` contient 40 lignes (2×4×5), une pour chaque combinaison de *sexe* (2 modalités), *groupe_ages* (4 modalités) et *etudes* (5 modalités)[^estimations-marginales-5]. La variable *heures.tv* est fixe et correspond à `mean(mf$heures.tv)`.

[^estimations-marginales-5]: Si vous avez une version de `{marginaleffects}` ≤ 0.8.1, vous obtiendrez 80 lignes en raison d'un bug qui dupliquait les lignes pour chaque modalité de *sport*.
...
Il est important de noter que le nom des colonnes n'est pas compatible avec les fonctions de `{broom.helpers}` et par extension avec `gtsummary::tbl_regression()` et `ggstats::ggcoef_model()`. On utilisera donc `broom.helpers::tidy_marginalcontrasts()`[^estimations-marginales-6] qui remets en forme le tableau de résultats dans un format compatible. On pourra ainsi produire un tableau propre des résultats[^estimations-marginales-7].

[^estimations-marginales-6]: Il existe également une fonction `broom.helpers::tidy_comparisons()` mais on lui préférera `broom.helpers::tidy_marginalcontrasts()`. Pour un modèle sans interaction, les résultats sont identiques. Mais `broom.helpers::tidy_marginalcontrasts()` peut gérer des termes d'interactions, ce qui sera utile dans un prochain chapitre (cf. @sec-interactions).

[^estimations-marginales-7]: Notez l'utilisation de `style_positive = "plus"` dans l'appel de `scales::label_percent()` pour ajouter un signe `+` devant les valeurs positives, afin de bien indiquer que l'on représente le résultat d'une différence.
