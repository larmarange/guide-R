...
::: callout-warning
Ce chapitre nécessite une version récente de `{broom.helpers}` (version ≥ 1.12.0), de `{gtsummary}` (version ≥ 1.6.3), de `{ggstats}` (version ≥ 0.2.1) et de `{marginaleffects}` (version ≥ 0.9.0).
:::
