...
::: callout-warning
Ce chapitre nécessite une version en cours de développement de `{broom.helpers}` installable avec la commande `remotes::install_github("larmarange/broom.helpers")`, ainsi qu'une version récente de `{gtsummary}` (version ≥ 1.6.3), de `{ggstats}` (version ≥ 0.2.1) et de `{marginaleffects}` (version ≥ 0.9.0).
:::
