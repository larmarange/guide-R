...
On peut ajouter facilement[^regression-logistique-binaire-4] cela au tableau produit avec gtsummary::tbl_regression() en ajoutant l'option add_pairwise_contrasts = TRUE.

[^regression-logistique-binaire-4]: Cela nécessite néanmoins au minimum la version 1.11.0 du package `{broom.helpers}` et la version 1.6.3 de `{gtsummary}`.
...
De même, on peur visualiser les coefficients avec la même option dans `ggstats::ggcoef_model()`[^regression-logistique-binaire-5]. On peut d'ailleurs choisir les variables concernées avec l'argument `pairwise_variables`.

[^regression-logistique-binaire-5]: Cela nécessite néanmoins au minimum la version 1.11.0 du package `{broom.helpers}` et la version 0.2.0 de `{ggstats}`.
...
Si l'on a recours à `gtsummary::tbl_regression()`, on peut facilement ajouter les p-valeurs globales avec `gtsummary::add_global_p()`[^regression-logistique-binaire-6].

[^regression-logistique-binaire-6]: Si l'on veut conserver les p-values individuelles associées à chaque *odds ratio*, on ajoutera l'option `keep = TRUE`.
...
La technique de sélection descendante pas à pas est une approche visant à améliorer son modèle explicatif[^regression-logistique-binaire-7]. On réalise un premier modèle avec toutes les variables spécifiées, puis on regarde s'il est possible d'améliorer le modèle en supprimant une des variables du modèle. Si plusieurs variables permettent d'améliorer le modèle, on supprimera la variable dont la suppression améliorera le plus le modèle. Puis on recommence le même procédé pour voir si la suppression d'une seconde variable peut encore améliorer le modèle et ainsi de suite. Lorsque le modèle ne peut plus être amélioré par la suppresion d'une variable, on s'arrête.

[^regression-logistique-binaire-7]: Il existe également des méthodes de *sélection ascendante pas à pas*, mais nous les aborderons pas ici.
