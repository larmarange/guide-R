...
### Représentations graphiques avec guideR
...
```{r}
trial |> 
  plot_continuous(age, by = c(grade, stage, trt))
```
...
trial |> 
  plot_continuous(
    age,
    by = c(grade, stage, trt),
    flip = TRUE,
    minimal = TRUE,
    mapping = ggplot2::aes(fill = by)
  ) +
  guideR::scale_fill_safe() +
...
On peut aussi utiliser directement les fonctions `guideR::mean_sd()` ou `guidesR::median_iqr()`.

```{r}
trial |> 
  mean_sd(age, .by = grade, .conf.int = TRUE)
trial |> 
  median_iqr(age, .by = grade)
```
