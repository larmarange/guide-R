...
Quand on travaille avec `{gtsumary}`, le plus facile est d'utiliser le thème `guideR::theme_gtsummary_fisher_simulate_p()` fourni par `{guideR}` et qui applique alors automatiquement le test de Fisher avec simulation de Monte Carlo pour toutes les variables catégorielles lorsque l'on fait appel à `add_p()`.
:::
