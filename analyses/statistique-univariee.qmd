...
```{r}
#| label: fig-diag-barres-personalise
...
#| tbl-cap: statisques personnalisées pour une variable continue
hdv2003 |>
  tbl_summary(
...
#| tbl-cap: statisques personnalisées pour une variable continue (2)
hdv2003 |>
  tbl_summary(
    include = c(age, heures.tv),
    statistic = list(
...
#| tbl-cap: statisques personnalisées pour une variable continue (3)
...
#| tbl-cap: statisques personnalisées pour une variable catégorielle
hdv2003 |>
  tbl_summary(
    include = occup,
...
[^statistique-univariee-3]: Notez l'utilisation de `rev()` pour inverser le tableau créé avec `table()` afin que le nombre de succès (`TRUE`) soit indiqués avant le nombre d'échecs (`FALSE`).

```{r}
table(hdv2003$trav.imp == "Peu important") |> 
  rev() |> 
  prop.test()
```
