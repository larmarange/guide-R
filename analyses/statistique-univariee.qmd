...
Le package `{guideR}`, le compagnon de *guide-R*, propose également quelques thèmes complémentaires. Notamment, `guideR::theme_gtsummary_prop_n()` permet d'afficher d'abord les proportions, en forcant le nombre de décimales à 1, suivi des effectifs entre parenthèses. Le thème `guideR::theme_gtsummary_bold_labels()` applique quant à lui du gras à tous les noms de variable.
