...
#| tbl-cap: Facteurs associés à l'absentéisme scolaire (modèle négatif binomial)
mod2_nb |> 
  ggcoef_table(exponentiate = TRUE)
```
...
Le package `{logbin}` propose, via `logib::logbin()`, une implémentation de la régression log-binomiale en proposant des algorithmes de convergence plus stables.
