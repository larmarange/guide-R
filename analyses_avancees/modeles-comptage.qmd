...
Nous voyons très clairement un décalage des deux distributions. Notre modèle de Poisson n'arrive pas à capturer la variabilité des observations. Faisons le test de surdispersion pour vérifier[^modeles-comptage-5].

[^modeles-comptage-5]: Il est possible que l'installation du package `{DHARMa}` soit requise pour la bonne réalisation du test.
...
Nous obtenons des résultats assez proches de ceux du modèle de Poisson. Notons cependant les différents avis affichés qui nous indiquent que le modèle a eu des difficultés à converger[^modeles-comptage-6].
...
Il est tout à fait possible d'appliquer `stats::step()` à ces modèles[^modeles-comptage-7].

[^modeles-comptage-7]: Y compris, dans le cas présent, au modèle de quasi-Poisson.
