...
Pour les résultats sous forme de tableau, nous pouvons avoir recours à `gtsummary::tbl_regression()`[^modeles-zero-inflated-2] et à l'utilitaire `guideR::grouped_tbl_pivot_wider()`[^modeles-zero-inflated-3].

[^modeles-zero-inflated-2]: À partir de la version 2.3.0.

[^modeles-zero-inflated-3]: Voir la section @sec-affichage-tableau-regression-multinomiale dans le chapitre sur la régression logistique multinomiale.

```{r}
#| label: tbl-tbl_regression-mod_zip
#| tbl-cap: Coefficients du modèle de Poisson *zero-inflated*
library(gtsummary)
mod_zip |> 
...
Nous pouvons également produire un tableau avec `gtsummary::tbl_regression()`[^modeles-zero-inflated-4].

[^modeles-zero-inflated-4]: À condition d'utiliser une version récente (2.3.0 minimum) de `{gtsummary}`.
...
Il devient alors plus facile d'utiliser `gtsummary::modify_spanning_header()` ou encore `gtsummary::modify_header()`.

```{r}
tbl_hurdle_nb |> 
  modify_spanning_header(
    ends_with("_1") ~ "**Composante comptage**",
    ends_with("_2") ~ "**Composante *zero-inflated***"
  ) |> 
  modify_header(
    estimate_1 ~ "**RR**",
    estimate_2 ~ "**OR**"
