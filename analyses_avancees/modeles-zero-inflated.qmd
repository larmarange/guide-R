...
Dans ce chapitre, nous allons utiliser un jeu de données issu d'un article de Partha Deb et Pravin K. Trivedi [@deb1997]. Ce jeu de données est disponible dans le package `{MixAll}` sous le nom de `DebTrivedi`. Il porte sur 4406 individus âgés de 66 ans ou plus et couvert par le programme américain *Medicare*.
