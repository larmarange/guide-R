...
Pour un graphique des coefficients, on peut appeler directement `ggstats::gcoef_model()` ou encore `ggstats::coef_table()`[^regression-logistique-multinomiale-2]. De plus, `{ggstats}` propose également les variantes `ggstats::gcoef_dodged()` et `ggstats::ggcoef_faceted()`.

[^regression-logistique-multinomiale-2]: Attention : pour que cela fonctionne avec un modèle multinomial, il est nécessaire d'utiliser la version 0.9.0 (ou une version plus récente) de `{ggstats}`.
