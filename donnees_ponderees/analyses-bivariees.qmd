...
De manière plus simple, on pourra utiliser le thème `guideR::theme_gtsummary_unweighted_n()` fourni par `{guideR}`, la package compagnon de *guide-R*. Cette fonction est plus complète et actualisera également la colonne total ou la présentation du nombre d'observations manquantes.

*Nota Bene :* cette fonction à également une option `mean_sd = TRUE` si, dans le même temps, on souhaite forcer l'affichage de la moyenne et de l'écart-type au lieu de la médiane et l'intervalle inter-quartile pour les variables continues.
...
```{r}
guideR::theme_gtsummary_unweighted_n()
...
::: callout-tip
Il faut noter que ni `{survey}` ni `{gtsummary}` ne fournissent nativement un test de comparaison de moyennes entre 3 groupes et plus (pour 2 groupes, il existe `survey::svyttest()`). `{guideR}` permet d'y remédier via une fonction `guideR::svyoneway()`. Ce test est directement disponible avec `{gtsummary}` en utilisant le theme `guideR::theme_gtsummary_unweighted_n(mean_sd = TRUE)`.
:::

## Calcul manuel avec `{guideR}`

Dans les chapitres sur la statistique univariée (cf. @sec-statistique-univariee) et la statistique bivariée (cf. @sec-statistique-bivariee), nous avions introduit les fonctions `guideR::mean_sd()`, `guideR::meadian_iqr()` et `guideR::proportion()`, fonctions fournies par `{guideR}`, le package compagnon de *guide-R*.

Or, ces trois fonctions sont également compatibles avec les objets `survey` et peuvent donc être utilisées de la même manière.

```{r}
dp |> 
  mean_sd(age, .by = region, .conf.int = TRUE)
dp |> 
  median_iqr(age, .by = region)
dp |> 
  proportion(educ, .conf.int = TRUE)
dp |> 
  proportion(educ, .by = region)
```
...
dp |> 
  group_by(region) |> 
  summarise(moy = survey_mean(age, vartype = "ci"))
```
...
Là encore, on peut passer l'option `vartype = "ci"` pour obtenir les intervalles de confiance[^analyses-bivariees-3].
