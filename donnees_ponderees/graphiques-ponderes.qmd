...
## avec les fonctions graphiques de `{guideR}`

Dans le chapitre sur la statistique bivariée (cf. @sec-statistique-bivariee), nous avons introduit plusieurs fonctions graphiques proposées par `{guideR}`, le package compagnon de *guide-R* : `guideR::plot_categorical()`, `guideR::plot_continuous()`, `guideR::plot_means()` et `guideR::plot_proportions()`. Or ces fonctions sont toutes compatibles avec les objets `survey` (y compris pour le calcul des intervalles de confiance et des tests de comparaison).

```{r}
library(guideR)
dp |> 
  plot_continuous(age, by = c(region, educ))
dp |> 
  plot_means(nb_enf_ideal, by = c(region, milieu, educ))
dp |> 
  plot_categorical(
    educ,
    by = c(region, milieu),
...
dp |> 
  plot_proportions(
    test == "oui",
    by = c(region, educ, milieu),
    show_overall = TRUE,
    fill = "lightblue"
  )
```

De même, les fonctions `guideR::plot_multiple_answers()` et `guideR::plot_multiple_answers_dodge()`, présentées dans le chapitre sur les questions à réponses multiples (cf. @sec-reponses-multiples), sont elles aussi compatibles avec les objets `survey`.

```{r}
dp |> 
  plot_multiple_answers(
    answers = c(journal, radio, tv),
    value = "oui",
