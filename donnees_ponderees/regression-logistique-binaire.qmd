...
La plupart du temps, les poids de pondération ne sont pas des nombres entiers, mais des nombres décimaux. Dès lors, on ne peut plus utiliser la famille de modèles binomiale (qui repose sur des nombres entiers de succès et d'échecs)[^regression-logistique-binaire-1]. On aura plutôt recours à la famille quasi-binomiale, que l'on spécifie avec `family = quasibinomial` et qui constitue une extension de la famille binomiale pouvant gérer des poids non entiers.

[^regression-logistique-binaire-1]: Si l'on indique `family = binomial`, vous obtiendrez avec une version récente de **R** un message d'avertissement du type `Avis : nombre de succès non entier dans un glm binomial !`. Avec une version plus ancienne de **R**, vous devriez même avoir un message d'erreur.
