...
```{r}
plot_scheme(colour("iridescent")(9), colours = TRUE, size = 0.9)
```
...
```{r}
plot_scheme(colour("nightfall")(9), colours = TRUE, size = 0.9)
```
...
-   pour 10 couleurs ou plus : utilisation de la palette *rainbow* avec un réordonnancent aléatoire des couleurs pour conserver le rendu catégoriel.
...
Masataka Okabe et Kei Ito ont développé en 2002 une palette de 9 couleurs adaptée aux personnes daltoniennes (<https://jfly.uni-koeln.de/color/>). Elle est notamment disponible via un package dédié, `{ggokabeito}` et ses fonctions dédiées comme `ggokabeito::scale_fill_okabe_ito()`. On la retrouve également dans le package `{see}` (cf. `see::scale_fill_okabeito()`). Elle est également accessible nativement sous R avec la fonction `grDevices::palette.colors(palette = "Okabe-Ito")`.

```{r}
library(ggokabeito)
scales::show_col(palette_okabe_ito())
```
