...
```{r}
gtsummary::trial |> guideR::view_dictionary()
```
:::
...
On peut également présenter les résultats sous forme d'un tableau formaté avec la fonction `labelled::to_gt()`, ce qui est idéal pour ensuite exporter le dictionnaire des variables dans un document externe avec `gt::gtsave()`.
