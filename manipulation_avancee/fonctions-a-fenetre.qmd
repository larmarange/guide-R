...
Une variante est la fonction `num_cycle()` ci-après[^fonctions-a-fenetre-1]. On doit lui passer une condition / vecteur logique en entrée. Il numérote uniquement les <q>sous-groupes</q> remplissant la condition et renvoie `NA` sinon.

[^fonctions-a-fenetre-1]: Ne pas oublier de copier également la fonction `is_different()`.
