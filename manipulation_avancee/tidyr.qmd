...
La fonction `tidyr::separate_rows()` est utile lorsque plusieurs valeurs sont contenues dans la même variable. Mais, alors que `tidyr::separate()` permet de répartir ces différentes valeurs dans plusieurs colonnes, `tidyr::separate_rows()` va créé une ligne pour chaque valeur. Prenons cet exemple trivial où les différentes notes de chaque élène sont contenues dans la colonne `notes`.
